...
DEFAULT_START_DATE = data.get("default_start_date", "2026-01-18")
DATA_LATENCY_DAYS = int(data.get("data_latency_days", 2))


START_DATE_OVERRIDE = data.get("start_date_override")       
CURRENT_DATE_OVERRIDE = data.get("current_date_override")
...
def get_composite_periods(start_date_str, current_date_str, interval_days=16):
    ...
    while cur <= current_date:
        end = cur + timedelta(days=interval_days - 1)

        # If the window crosses a year boundary, force the end to Jan 1 of the next year
        if end.year != cur.year:
            end = date(cur.year + 1, 1, 1)

        # Only include complete periods up to current_date
        if end > current_date:
            break

        periods.append({'start': cur, 'end': end})
...
def download_tamsat_data(date_str, save_path):
    ...
    # Avoid re-downloading files we already have
    if os.path.exists(save_path) and os.path.getsize(save_path) > 0:
        print(f"File already exists for {date_str}. Skipping: {save_path}")
        return
    try:
...
# ----------------------------
# AUTOMATED DATES (16-day windows)
# - start_date_ continues from the last processed composite end date + 1 day
#   (uses latest raster in biomass_path only)
# - current_date is set to the end date of the last FULL 16-day period available
#   up to today (optionally minus DATA_LATENCY_DAYS)
#
# Optional overrides (environment variables):
#   DEFAULT_START_DATE=YYYY-MM-DD       # fallback if no prior outputs/rasters exist
#   DATA_LATENCY_DAYS=2                 # buffer for data availability lag
#   START_DATE_OVERRIDE=YYYY-MM-DD      # force a specific start date
#   CURRENT_DATE_OVERRIDE=YYYY-MM-DD    # force a specific current/end date
# ----------------------------

DEFAULT_START_DATE = os.getenv("DEFAULT_START_DATE", DEFAULT_START_DATE)
DATA_LATENCY_DAYS = int(os.getenv("DATA_LATENCY_DAYS", str(DATA_LATENCY_DAYS)))
START_DATE_OVERRIDE = os.getenv("START_DATE_OVERRIDE", START_DATE_OVERRIDE or "") or None
CURRENT_DATE_OVERRIDE = os.getenv("CURRENT_DATE_OVERRIDE", CURRENT_DATE_OVERRIDE or "") or None

def _parse_yyyymmdd(s: str) -> date:
    return datetime.strptime(s, "%Y%m%d").date()


def _last_date_from_biomass_rasters(folder: str):
    try:
        ...
        for fn in os.listdir(folder):
            ...
            if not m:
                continue
            try:
                dates.append(_parse_yyyymmdd(m.group(1)))
            except ValueError:
                pass
...
# Determine start_date_ (as a date object)
if START_DATE_OVERRIDE:
    start_date_obj = datetime.strptime(START_DATE_OVERRIDE, "%Y-%m-%d").date()
else:
    last_end = _last_date_from_biomass_rasters(biomass_path)
    if last_end:
        start_date_obj = last_end + timedelta(days=1)
    else:
        start_date_obj = datetime.strptime(DEFAULT_START_DATE, "%Y-%m-%d").date()

# Determine current_date (end date of the last COMPLETE 16-day window)
if CURRENT_DATE_OVERRIDE:
    current_date_obj = datetime.strptime(CURRENT_DATE_OVERRIDE, "%Y-%m-%d").date()
else:
    today_obj = date.today() - timedelta(days=DATA_LATENCY_DAYS)
    probe_periods = get_composite_periods(
        start_date_obj.strftime("%Y-%m-%d"),
        today_obj.strftime("%Y-%m-%d")
    )
    if not probe_periods:
        print(f"No full 16-day period available yet since {start_date_obj} (today={date.today()}, latency={DATA_LATENCY_DAYS}d).")
        exit_program()
    current_date_obj = probe_periods[-1]["end"]

start_date_ = start_date_obj.strftime("%Y-%m-%d")
current_date = current_date_obj.strftime("%Y-%m-%d")

print(f"Auto start_date_  : {start_date_}")
print(f"Auto current_date : {current_date}")

# end_date_obj should be a datetime.date object for downstream loops
...
for period in composite_periods:
    ...
    sm = getImage("NASA/SMAP/SPL4SMGP/008", start_date_str, end_date_str, "sm_surface")
